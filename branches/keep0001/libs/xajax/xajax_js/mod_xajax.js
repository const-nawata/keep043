/* xajax Javascript library :: version 0.2.4 */var ifxAjaxError = 1;
function alertMy(s){if(ifxAjaxError==1)alert(s);}
...
text=text.replace(/</g,"&lt;")
text=text.replace(/>/g,"&gt;")
debugTag=this.debugWindow.document.getElementById('debugTag');debugTag.innerHTML=('<b>'+(new Date()).toString()+'</b>: '+text+'<hr/>')+debugTag.innerHTML;}catch(e){alertMy("Xajax Debug:\n "+text);}
};this.workId='xajaxWork'+new Date().getTime();this.depth=0;this.responseErrorsForAlert=["400","401","402","403","404","500","501","502","503"];
...
this.getRequestObject=function(){
    ...
    var req=null;
    if(typeof XMLHttpRequest!="undefined"){/*alert('new XMLHttpRequest');*/ req=new XMLHttpRequest();}
...
//var o = {}
/*
for(att in req.responseXML){
alert(req.responseXML[att]);    
    }
*/
...
alertMy("Illegal request type: "+xajaxRequestType);return false;break;}
...
catch(e){alertMy("Your browser does not appear to  support asynchronous requests using POST.");return false;}
}
...
r.onreadystatechange=function(){
    ...
    //alert('r.readyState: '+r.readyState+' # r.status: '+r.status+' # xajaxDebug: '+xajaxDebug);
...
//alert('r.readyState: '+r.readyState+' # r.status: '+r.status+' # xajaxDebug: '+xajaxDebug);
//alert('r.responseXML: '+r.responseXML+' # r.responseXML.documentElement'+r.responseXML.documentElement);
...
if(cmd=="al"){cmdFullname="addAlert";alertMy(data);}
...
alertMy("While trying to '"+cmdFullname+"' (command number "+i+"), the following error occured:\n"
+e.name+": "+e.message+"\n"
