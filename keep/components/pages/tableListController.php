...
//______________________________________________________________________________
	...
	private function resetJsHandlers( &$objResponse, $info ){
		...
		$class	= $info['class'];
		$page_obj	= new $class( NULL );
		$handlers	= array();
		$handlers	= $page_obj->getNotSelectedSellHanlders( $info['old_code'], $info['old_prompt'] );
		$div_view	= SeveralTablesPage::getSellHtmlContent( $info['old_code'], $info['old_prompt'], $handlers, 'SeveralTablesPageMenuSellTd' );
		$objResponse->assign( 'td_sell_'. $info['old_code'], 'innerHTML', $div_view );

		$handlers	= array();
		$div_view	= SeveralTablesPage::getSellHtmlContent( $info['new_code'], $info['new_prompt'], $handlers, 'SeveralTablesPageMenuSelectedSellTd' );
